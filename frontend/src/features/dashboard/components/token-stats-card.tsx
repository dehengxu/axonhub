...
export function TokenStatsCard() {
  ...
  const totals = useMemo(() => {
    if (!detailedModelStats?.currentPeriod || detailedModelStats.currentPeriod.length === 0) {
      return null;
    }
    return detailedModelStats.currentPeriod.reduce(
      (acc, curr) => ({
        totalPromptTokens: acc.totalPromptTokens + curr.totalInputTokens,
        totalCompletionTokens: acc.totalCompletionTokens + curr.totalOutputTokens,
                ...
                {isLoadingModelStats ? null : totals ? (
                  <Alert className='mt-4'>
                    ...
                    <div className='grid grid-cols-3 gap-4 mt-2'>
                      ...
                      <div>
                        <div className='text-sm text-muted-foreground'>{t('dashboard.stats.totalCompletionTokens')}</div>
                        <div className='text-xl font-bold'>{formatNumber(totals.totalCompletionTokens)}</div>
                      </div>
                      <div>
